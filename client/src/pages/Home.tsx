...
const channels = [
  ...
  { id: "instagram", label: "Carrusel IG", icon: Layers3, accent: "bg-[#ff9aaf]", description: "Idea visual por slides", soon: false },
...
const demoSlides: CarouselSlide[] = ["Hook", "Contexto", "Idea central", "Desarrollo", "Aprendizaje", "CTA"].map((eyebrow, index) => ({ order: index + 1, eyebrow, title: index === 0 ? "Una idea que detiene el scroll" : `Slide ${index + 1} del carrusel`, body: "Texto breve, editable y fácil de leer en formato 4:5.", visualHint: "Composición oscura con acento lima" }));
const demoCarousel = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("demo") === "carousel";
const demoLibrary = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("demo") === "library";
type CarouselSlide = { order: number; eyebrow?: string; title: string; body: string; visualHint?: string };
const parseSlides = (value?: string | null): CarouselSlide[] => { try { return value ? JSON.parse(value) as CarouselSlide[] : []; } catch { return []; } };

export default function Home() {
  const { user, loading, isAuthenticated } = useAuth();
  const [tab, setTab] = useState<"create" | "drafts" | "profile">("create");
  const [step, setStep] = useState<Step>(demoCarousel ? 4 : 1);
  const [channel, setChannel] = useState<ChannelId>(demoCarousel ? "instagram" : "linkedin");
  const [format, setFormat] = useState<"post" | "carousel" | "short-video">(demoCarousel ? "carousel" : "post");
  ...
  const [content, setContent] = useState(demoCarousel ? "Caption de ejemplo para editar antes de publicar." : "");
  const [notice, setNotice] = useState("");
  const [showChannelPicker, setShowChannelPicker] = useState(false);
  const [copied, setCopied] = useState(false);
  const [draftFilter, setDraftFilter] = useState<"all" | "draft" | "archived">("all");
  const [draftChannel, setDraftChannel] = useState<"all" | "instagram">("all");
  const [slides, setSlides] = useState<CarouselSlide[]>(demoCarousel ? demoSlides : []);
  const [activeSlide, setActiveSlide] = useState(0);

  const demoDrafts = useMemo(() => [{ id: 99, title: "Carrusel guardado", idea: "Idea validada", content: "Caption editable", channel: "instagram", format: "carousel", status: "draft", updatedAt: new Date().toISOString(), slides: JSON.stringify(demoSlides) }], []);
  const draftsQuery = trpc.content.listDrafts.useQuery(undefined, { enabled: isAuthenticated && tab === "drafts" });
  const generateMutation = trpc.content.generate.useMutation({
    onSuccess: (result) => {
      setTitle(result.title);
      setContent(result.caption || `${result.hook}\n\n${result.body}\n\n${result.callToAction}\n\n${result.hashtags.join(" ")}`);
  ...
  const visibleDrafts = useMemo(() => (demoLibrary ? demoDrafts : (draftsQuery.data ?? [])).filter((draft) => (draftFilter === "all" || draft.status === draftFilter) && (draftChannel === "all" || draft.channel === draftChannel)), [demoDrafts, demoLibrary, draftsQuery.data, draftFilter, draftChannel]);
  ...
  const save = () => requireAuth(() => saveMutation.mutate({ title, idea: idea || title, channel, format, objective, audience, tone, coreMessage: coreMessage || idea, callToAction, content: content || starterContent, slides: slides.length ? JSON.stringify(slides) : undefined }));
  ...
  const updateSlide = (patch: Partial<CarouselSlide>) => setSlides((items) => items.map((item, index) => index === activeSlide ? { ...item, ...patch } : item));
  const duplicateSlide = () => { const source = slides[activeSlide]; if (!source) return; const copy = { ...source, order: source.order + 1, title: `${source.title} · copia` }; setSlides((items) => [...items.slice(0, activeSlide + 1), copy, ...items.slice(activeSlide + 1)].map((item, index) => ({ ...item, order: index + 1 }))); setActiveSlide(activeSlide + 1); };
  const addSlide = () => { const next = { order: slides.length + 1, eyebrow: "NUEVA IDEA", title: "Una idea que merece espacio", body: "Edita este texto para completar tu carrusel.", visualHint: "Composición limpia con acento lima" }; setSlides((items) => [...items, next]); setActiveSlide(slides.length); };
  const removeSlide = () => { if (slides.length <= 1) return; setSlides((items) => items.filter((_, index) => index !== activeSlide).map((item, index) => ({ ...item, order: index + 1 }))); setActiveSlide((index) => Math.max(0, Math.min(index, slides.length - 2))); };
      ...
      <main className="studio-main">
        {tab === "create" && <>
          ...
          <section className="flow-card" aria-label="Flujo de creación">
            ...
            {step === 4 && (channel === "instagram" ? <div className="step-content carousel-editor"><div className="carousel-toolbar"><span><Layers3 size={14} /> CARRUSEL · 4:5</span><strong>{slides.length ? `${activeSlide + 1} / ${slides.length}` : "listo para crear"}</strong></div>{slides.length ? <><div className="carousel-frame"><div className="carousel-frame-top"><span>{slides[activeSlide]?.eyebrow || "IDEA CENTRAL"}</span><span>IG / {String((slides[activeSlide]?.order || 1)).padStart(2, "0")}</span></div><Input value={slides[activeSlide]?.title || ""} onChange={(event) => updateSlide({ title: event.target.value })} aria-label="Título de la diapositiva" className="slide-title" /><Textarea value={slides[activeSlide]?.body || ""} onChange={(event) => updateSlide({ body: event.target.value })} aria-label="Texto de la diapositiva" className="slide-body" /><div className="slide-visual"><Sparkles size={15} /><span>{slides[activeSlide]?.visualHint || "Sugerencia visual"}</span></div></div><div className="slide-actions"><button onClick={() => setActiveSlide((index) => Math.max(0, index - 1))} disabled={activeSlide === 0} aria-label="Diapositiva anterior"><ChevronRight className="rotate-180" size={17} /></button><button onClick={addSlide} aria-label="Añadir diapositiva"><Plus size={17} /></button><button onClick={duplicateSlide} aria-label="Duplicar diapositiva"><Copy size={14} /></button><button onClick={removeSlide} disabled={slides.length <= 1} aria-label="Eliminar diapositiva"><X size={16} /></button><button onClick={() => setActiveSlide((index) => Math.min(slides.length - 1, index + 1))} disabled={activeSlide === slides.length - 1} aria-label="Diapositiva siguiente"><ChevronRight size={17} /></button></div><div className="slide-strip" aria-label="Navegar entre diapositivas">{slides.map((slide, index) => <button key={`${slide.order}-${index}`} className={index === activeSlide ? "active" : ""} onClick={() => setActiveSlide(index)} aria-label={`Ir a diapositiva ${index + 1}`}><span>{String(index + 1).padStart(2, "0")}</span><small>{slide.title.slice(0, 18)}</small></button>)}</div><label className="field-label" htmlFor="carousel-caption">Caption</label><Textarea id="carousel-caption" value={content} onChange={(event) => setContent(event.target.value)} className="carousel-caption" aria-label="Editar caption del carrusel" /></> : <div className="carousel-empty"><Layers3 size={25} /><strong>Tu carrusel aparecerá aquí.</strong><span>Completa el brief y genera seis slides editables.</span></div>}</div> : <div className="step-content preview-step"><div className="preview-head"><div className="mini-channel"><span className="mini-avatar">C</span><span><strong>Cristhian S.I.R.</strong><small>Product Builder · ahora</small></span></div><span className="preview-network"><SelectedChannelIcon size={15} /> {selectedChannel.label.toUpperCase()}</span></div><Input value={title} onChange={(event) => setTitle(event.target.value)} className="preview-title" aria-label="Título del borrador" /><Textarea value={content || starterContent} onChange={(event) => setContent(event.target.value)} className="preview-editor" aria-label="Editar borrador" /><div className="preview-footer"><span><Hash size={14} /> {content.split(/\s+/).filter(Boolean).length || 0} palabras</span><span className="editable-label"><PenLine size={13} /> editable</span></div></div>)}
